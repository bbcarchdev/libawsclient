...
static int verbose;
aws_signature_version authversion;
...
main(int argc, char **argv)
{
	...
	s3 = aws_s3_create_uristr(bucketuri);
	if(!s3)
	{
		fprintf(stderr, "%s: failed to create S3 descriptor from <%s>\n", short_progname, bucketuri);
		return EXIT_FAILURE;
	}
	if(verbose)
	{
	...
	if(accesskey)
	{
		aws_s3_set_access(s3, accesskey);
	}
	if(secretkey)
	{
		aws_s3_set_secret(s3, secretkey);
	}
	if(endpoint)
	{
		aws_s3_set_endpoint(s3, endpoint);
	}
	if(token)
	{
		aws_s3_set_token(s3, token);
	}
	if(region)
	{
		aws_s3_set_region(s3, region);
	}
	if(authversion)
	{
		aws_s3_set_version(s3, authversion);
	}
