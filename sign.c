...
static struct curl_slist *aws_request_headers_including_reqd_(const AWSSIGN * restrict sign, struct curl_slist *headers) AWS_MALLOC;
static struct curl_slist *aws_create_reqd_headers_(const AWSSIGN * restrict sign) AWS_MALLOC;
static struct curl_slist *aws_create_auth_headers_(const AWSSIGN * restrict sign, struct curl_slist * restrict headers) AWS_MALLOC;
static char *aws_create_amz_content_hash_header_(char *value) AWS_MALLOC;
static char *aws_create_authentication_v4_header_(const AWSSIGN * restrict sign, struct curl_slist * restrict headers) AWS_MALLOC;
static char *aws_header_credentials_(const AWSSIGN *sign) AWS_MALLOC;
static char *aws_credential_scope_(const AWSSIGN *sign) AWS_MALLOC;
static char *aws_signed_header_names_(struct curl_slist *headers) AWS_MALLOC;
static char *aws_signature_v4_(const AWSSIGN * restrict sign, struct curl_slist * restrict headers) AWS_MALLOC;
static uint8_t *aws_signature_v4_digest_(const AWSSIGN * restrict sign, struct curl_slist * restrict headers) AWS_MALLOC;
static uint8_t *aws_derived_signing_key_(const AWSSIGN *sign) AWS_MALLOC;
static char *aws_string_to_sign_(const AWSSIGN * restrict sign, struct curl_slist * restrict headers) AWS_MALLOC;
static char *aws_canonical_request_description_hex_(const AWSSIGN * restrict sign, struct curl_slist * restrict headers) AWS_MALLOC;
static char *aws_canonical_request_description_(const AWSSIGN * restrict sign, struct curl_slist * restrict headers) AWS_MALLOC;
static char *aws_canonical_uri_(const AWSSIGN *sign) AWS_MALLOC;
static char *aws_normalised_path_(char *path) AWS_MALLOC;
static char *aws_normalised_resource_key_(char *resource_key) AWS_MALLOC;
static char *aws_canonical_query_string_(const AWSSIGN *sign) AWS_MALLOC;
static char *aws_canonical_headers_description_(struct curl_slist *headers) AWS_MALLOC;
static char *aws_canonical_header_(char *h) AWS_MALLOC;
static char *aws_canonical_header_name_(char *h) AWS_MALLOC;
static char *aws_canonical_header_value_(char *h) AWS_MALLOC;
static char *aws_hex_(size_t length, const uint8_t *bytes) AWS_MALLOC;
static uint8_t aws_unhex_byte_(char high, char low) AWS_CONST;
static uint8_t *aws_sha256_(const char *str) AWS_MALLOC;
static uint8_t *aws_sha256_binary_(size_t data_length, const uint8_t *data) AWS_MALLOC;
static uint8_t *aws_hmac_sha256_(size_t secret_key_length, const uint8_t * restrict secret_key, char * restrict str) AWS_MALLOC;
static char *aws_create_host_header_(const char *host) AWS_MALLOC;
static int aws_header_sort_(const void *a, const void *b) AWS_PURE;
