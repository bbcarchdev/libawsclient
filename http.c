...
aws_set_http_header(
	mutable struct curl_slist * const nullable restrict headers,
	const char * const nonnull restrict header
) {
 ...
 */
const struct curl_slist *
list_with_http_header(const struct curl_slist * const nullable restrict headers, const char * const nonnull restrict header)
{
	return aws_set_http_header((struct curl_slist *) aws_curl_slist_copy(headers), header);
}
 ...
 */
const char *
aws_http_header_name(const char * const header)
{
 ...
 */
const char *
aws_http_header_value(const char * const header)
{
...
size_t
aws_http_header_name_length(const char * const header)
{
//	return header ? MAX(0, strchr(header, ':') - header) : 0;

	if(!header) return 0;
	const ptrdiff_t len = strchr(header, ':') - header;
	if(len < 0) return 0;
	return len;
}
 ...
 */
const char *
aws_create_http_date_header(const time_t * const nonnull timestamp)
{
	char *date = (char *) aws_http_date(timestamp);
	if(!date) return NULL;
	const char * const h = aws_strf("Date: %s", date);
 ...
 */
const char *
aws_http_date(const time_t * const nonnull timestamp)
{
...
const char *
aws_http_date_tm(const struct tm * const nonnull time)
{
	if(!time) return NULL;

	// replace %a and %b in date format string
	// this is safer than changing the global locale to "C"
	const char * const days[] = {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"};
	const char * const months[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};

	char *src = HTTP_DATE_FORMAT, *dst = alloca(strlen(HTTP_DATE_FORMAT) + 3), *fmt = dst;
	while(*src) {
		if(*src == '%' && *(src+1) == 'a') {
